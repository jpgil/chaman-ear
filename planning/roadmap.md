...
- [ ] **Clase `ChamanEar`** (interfaz pública según specs sección 1.1):
  ...
  - [ ] Test: `onPartial` se dispara al recibir evento `partial` del servidor.
  - [ ] Test: `onFinal` se dispara al recibir evento `final` del servidor.
  - [ ] Test: `onError` se dispara al recibir evento `error` del servidor.
  - [ ] Test: `onStateChange` refleja transiciones de estado correctas.
- [ ] **Captura de audio:**
  - [ ] Test: `MediaRecorder` captura audio en formato WebM/Opus (o el formato definido en Fase 0).
  - [ ] Test: los chunks se envían cada `chunkInterval` ms por WebSocket.
- [ ] **Mensajes WebSocket:**
  - [ ] Test: el mensaje `start` contiene `context` y `formatInstruction`.
  - [ ] Test: `formatInstruction` usa el valor por defecto si se omite.
  - [ ] Test: los chunks de audio se envían como mensajes binarios.
...
- [ ] **Manejo de `stop`:**
  - [ ] Test: al recibir `stop`, fuerza evento `final` con texto pendiente.
...
- [ ] **Estructura HTML:**
  ...
  - [ ] Zona de texto para la salida (parcial en gris, final en blanco).
...
- [ ] **Integración con el SDK:**
  ...
  - [ ] Conectar `onPartial` -> renderizado en gris/cursiva.
  - [ ] Conectar `onFinal` -> solidificar texto en blanco.
...
- Se puede hablar, ver texto parcial y texto final en pantalla.
...
- [ ] **Tests de integración:**
  - [ ] Test E2E: flujo completo start -> audio -> partial -> final -> stop.
...
- [ ] **Validación de rendimiento:**
  - [ ] Medir latencia percibida: tiempo entre habla y aparición del primer `partial`.
  - [ ] Medir latencia de `final`: tiempo entre final del habla y solidificación del texto.
- [ ] **Documentación:**
...
- La latencia percibida es inferior a 1 segundo para el primer `partial`.
